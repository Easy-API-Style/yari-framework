...
final class TokenizerMaps {
    ...
    /**
     * A {@link Function} that recognizes a scientific notation and tokenizes to a {@link ScientificNotation}.
     */
    static final Function<String, ScientificNotation> SCIENTIFIC_NOTATION = new Function<String, ScientificNotation>() {
        @Override
        public ScientificNotation apply(String text) {
    ...
    /**
     * A {@link Function} that recognizes a string literal quoted by double quote character ({@code "}) 
     * and tokenizes to a {@code String}. 
     * The backslash character ({@code \}) is interpreted as escape.
     */
    ...
    /**
     * A {@link Function} that tokenizes a SQL style string literal quoted by single quote character ({@code '}) 
     * and tokenizes to a {@code String}. 
     * Two adjacent single quote characters ({@code ''}) are escaped as one single quote character.
     */
    ...
    /**
     * A {@link Function} that recognizes a character literal quoted by single quote character ({@code '} 
     * and tokenizes to a {@link Character}. 
     * The backslash character ({@code \}) is interpreted as escape.
     */
    ...
    /**
     * Returns a map that tokenizes the recognized character range 
     * to a {@link Tokens.Fragment} object tagged with {@code tag}.
     */
    static Function<String, Fragment> fragment(final Object tag) {
        return new Function<String, Fragment>() {
            @Override
            public Fragment apply(String text) {
