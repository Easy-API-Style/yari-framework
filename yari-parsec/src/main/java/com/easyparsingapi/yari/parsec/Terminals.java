...
import com.easyparsingapi.yari.parsec.Tokens.ScientificNotation;
 ...
 * The following example is a parser snippet for Java generic type expression such as {@code List<String>}: 
 * <pre>{@code
...
public final class Terminals extends Lexicon {
    ...
    public static final class CharLiteral {
        ...
        public static final Parser<Character> SINGLE_QUOTE_TOKENIZER = 
            Scanners.SINGLE_QUOTE_CHAR
                     .map(TokenizerMaps.SINGLE_QUOTE_CHAR);
    }
    ...
    public static final class StringLiteral {
        ...
        public static final Parser<String> DOUBLE_QUOTE_TOKENIZER =
            Scanners.DOUBLE_QUOTE_STRING
                    .map(TokenizerMaps.DOUBLE_QUOTE_STRING);

        /**
         * A tokenizer that parses single quoted string literal (single quote is escaped with another single quote), 
         * and transforms the quoted content by applying escape characters.
         */
        public static final Parser<String> SINGLE_QUOTE_TOKENIZER =
            Scanners.SINGLE_QUOTE_STRING
                    .map(TokenizerMaps.SINGLE_QUOTE_STRING);
    }
    ...
    public static final class LongLiteral {
        ...
        /**
         * A tokenizer that parses a octal integer number (valid patterns are: {@code 0, 07, 017, 0371} etc.), 
         * and converts the string to a {@link Long} value.
         * 
         * <p>
        ...
        /**
         * A tokenizer that parses a hex integer number (valid patterns are: {@code 0x1, 0Xff, 0xFe1} etc.), 
         * and converts the string to a {@link Long} value.
         * 
         * <p>
    ...
    public static final class ScientificNumberLiteral {
        ...
        public static final Parser<ScientificNotation> PARSER = Parsers.tokenType(ScientificNotation.class,
        ...
        public static final Parser<ScientificNotation> TOKENIZER = 
            Scanners.SCIENTIFIC_NOTATION
                    .map(TokenizerMaps.SCIENTIFIC_NOTATION);
    }
    ...
    /**
     * Returns a {@link Terminals} object for lexing the operators with names specified in {@code ops}. 
     * Operators are lexed as {@link Tokens.Fragment} with {@link Tag#RESERVED} tag. 
     * 
     ...
     *
     * <p>
     * By default identifiers are recognized through {@link #identifier} during token-level parsing phase. 
     * Use {@link Builder#tokenizeWordsWith} to tokenize differently, 
     * and choose an alternative token-level parser accordingly.
     *
     * @param wordScanner defines words recognized by the new instance
     * @return a new {@link Builder} configured with the given word scanner
     */
     ...
     *   ...
     *   Parser<?> parser = grammar.from(terms.tokenizer().or(IntegerLiteral.TOKENIZER), Scanners.WHITSPACES.optional());
     * }</pre>
     *
     */
    public final class Builder {
         ...
         *
         * <p>
         * Note that if you call {@link #keywords} or {@link #caseInsensitiveKeywords} multiple times
         * on the same {@link Builder} instance, the last call overwrites previous calls.
         *
         * @param keywords the keyword strings to register
         ...
         * on the same {@link Builder} instance, the last call overwrites previous calls.
         *
         * @param keywords the collection of keyword strings to register
         * @return this {@link Builder} instance for chaining
         */
        ...
        /**
         * Defines case insensitive keywords. Keywords are special words with their own grammar rules.
         * To get the parser for a keyword, call {@code token(keyword)}.
         *
         * <p>
         * Note that if you call {@link #keywords} or {@link #caseInsensitiveKeywords} multiple times
         * on the same {@link Builder} instance, the last call overwrites previous calls.
         *
         * @param keywords the keyword strings to register for case-insensitive matching
         * @return this {@link Builder} instance for chaining
         */
         ...
         * on the same {@link Builder} instance, the last call overwrites previous calls.
         *
         * @param keywords the collection of keyword strings to register for case-insensitive matching
         * @return this {@link Builder} instance for chaining
         */
        ...
        /**
         * Configures an alternative tokenization strategy for words (except keywords).
         *
         * @param wordMap a function that converts a scanned word string into its token value
         * @return this {@link Builder} instance for chaining
         */
         ...
         *
         * @return a new {@link Terminals} instance combining operator recognition with the configured word rules
         */
        public Terminals build() {
            return new Terminals(union(Keywords.lexicon(wordScanner, keywords, stringCase, wordTokenMap)));
        }
     ...
     *
     * @return a {@link Parser} that yields the text of any identifier token
     */
    ...
    /**
     * Returns a {@link Parser} that recognizes {@link Tokens.Fragment} token values tagged
     * with one of {@code tags}.
     *
     * @param tags the acceptable fragment tags; a token is accepted if its tag matches any one of these
     * @return a {@link Parser} that yields the text of any matching {@link Tokens.Fragment} token
     */
    ...
    /**
     * Returns a {@link TokenMap} object that only recognizes {@link Tokens.Fragment} token values tagged 
     * with one of {@code tags}.
     */
