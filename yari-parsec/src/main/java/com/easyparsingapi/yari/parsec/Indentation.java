...
/**
 * Processes indentation based lexical structure according 
 * to the <a href="http://en.wikipedia.org/wiki/Off-side_rule">Off-side rule</a>.
 */
public final class Indentation {
    ...
    /**
     * A {@link Pattern} object that matches a line continuation. i.e. a backslash character ({@code '\'}) 
     * followed by some whitespaces and ended by a line feed character ({@code '\n'}). 
     * Is useful if the line feed character plays a role in the syntax (as in indentation-sensitive languages) 
     * and line continuation is supported.
     */
    static final Pattern LINE_CONTINUATION = 
        Patterns.sequence(Patterns.isChar('\\'), 
                          Patterns.many(INLINE_WHITESPACE),
                          Patterns.isChar('\n'));
    ...
    public static final Parser<Void> WHITESPACES = 
        INLINE_WHITESPACES.or(LINE_CONTINUATION)
                          .many1()
    ...
    /**
     * Creates an {@link Indentation} object that uses {@code indent}
     * and {@code outdent} as the token values for indentation and outdentation.
     *
     * @param indent  the token value used to represent an indentation
     * @param outdent the token value used to represent an outdentation
     */
    public Indentation(Object indent,
                       Object outdent) {
        this.indent = indent;
     ...
     *
     * @return a parser that matches the indent token
     */
     ...
     *
     * @return a parser that matches the outdent token
     */
    ...
    /**
     * A {@link Parser} that greedily runs {@code tokenizer},
     * and translates line feed characters ({@code '\n'}) to {@code indent} and {@code outdent} tokens.
     ...
     *
     * @param tokenizer the tokenizer parser to run on the input
     * @param delim     the delimiter parser whose matches are ignored
     * @return a parser that produces a list of tokens with indentation tokens inserted
     */
    public Parser<List<Token>> lexer(Parser<?> tokenizer,
                                     Parser<?> delim) {
        Parser<?> lf = Scanners.isChar('\n').retn(Punctuation.LF);
        return Parsers.or(tokenizer, lf)
                      .lexer(delim)
     ...
     */
    List<Token> analyzeIndentations(List<Token> tokens, 
                                    Object lf) {
        if (tokens.isEmpty()) {
            return tokens;
        }
    ...
    private static Token pseudoToken(int index,
                                     Object value) {
        return new Token(index, 0, value, null);
    }
