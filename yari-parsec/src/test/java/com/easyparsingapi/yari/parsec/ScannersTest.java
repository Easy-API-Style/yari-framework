...
import static com.easyparsingapi.yari.parsec.Asserts.assertScanner;
import static com.easyparsingapi.yari.parsec.Asserts.assertStringScanner;
...
public class ScannersTest {
    ...
    public void testBlockComment_withParsers() {
        Parser<Void> scanner = Scanners.blockComment(Scanners.string("<!--"), Scanners.string("-->"), Scanners.ANY_CHAR);
        assertNull(scanner.parse("<!--abc-->"));
        assertNull(scanner.parse("<!---->"));
        assertFailure(mode, scanner, "", 1, 1);
        assertFailure(mode, scanner, "a", 1, 1);
    ...
    public void testBlockComment_withQuotedParserThatMatchesEmpty() {
        Parser<Void> scanner = Scanners.blockComment(Scanners.string("<!--"), Scanners.string("-->"), Patterns.ALWAYS.toScanner("nothing"));
        assertNull(scanner.parse("<!---->"));
        assertFailure(mode, scanner, "", 1, 1);
        assertFailure(mode, scanner, "<!-", 1, 1);
    }
    ...
    public void testBlockComment_withQuotedParserThatMismatches() {
        Parser<Void> scanner = Scanners.blockComment(Scanners.string("<!--"), Scanners.string("-->"), Patterns.NEVER.toScanner("nothing"));
        assertNull(scanner.parse("<!---->"));
        assertFailure(mode, scanner, "", 1, 1);
        assertFailure(mode, scanner, "<!-", 1, 1);
    }
    ...
    public void testNestableBlockComment_withQuotedParser() {
        Parser<Void> scanner = Scanners.nestableBlockComment(Scanners.string("<!--"), Scanners.string("-->"), Scanners.isChar(CharPredicates.not(CharPredicates.IS_DIGIT)));
        assertNull(scanner.parse("<!-- not nested -->"));
        assertNull(scanner.parse("<!-- this is <!--nested--> -->"));
        assertFailure(mode, scanner, "", 1, 1);
    ...
    public void testNestedBlockComment_partialMatch() {
        Parser<Void> scanner = Scanners.nestableBlockComment(areChars("/*"), areChars("*/"), Scanners.isChar('a').many());
    ...
    public void testNestableBlockComment_openQuoteConsumesNoChar() {
        Parser<Void> scanner = Scanners.nestableBlockComment(Parsers.always(), Scanners.string("*/"), Scanners.ANY_CHAR);
        assertFailure(mode, scanner, "/**/", 1, 1, IllegalStateException.class);
    }

    @Test
    public void testNestableBlockComment_closeQuoteConsumesNoChar() {
        Parser<Void> scanner = Scanners.nestableBlockComment(Scanners.string("/*"), Parsers.always(), Scanners.ANY_CHAR);
    ...
    public void testNestedScanner() {
        Parser<Void> scanner = Scanners.nestedScanner(Scanners.isChar(CharPredicates.IS_ALPHA).skipMany1(), Scanners.isChar('a').skipTimes(2));
