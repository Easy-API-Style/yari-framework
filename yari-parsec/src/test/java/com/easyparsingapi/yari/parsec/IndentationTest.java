...
import com.easyparsingapi.yari.parsec.Indentation;
...
public class IndentationTest {
    ...
    public void testAnalyzeIndentations() {
        ...
        assertEquals(tokenList("foo", 2, Indentation.Punctuation.INDENT, "bar", 2, "bar2", 4, Indentation.Punctuation.INDENT, "baz", Indentation.Punctuation.OUTDENT, Indentation.Punctuation.OUTDENT),
                     analyze("foo", "\n", 1, "bar", "\n", 1, "bar2", "\n", 3, "baz"));
        assertEquals(tokenList("foo", 2, Indentation.Punctuation.INDENT, "bar", 4, Indentation.Punctuation.INDENT, "baz", 3,
                               Indentation.Punctuation.OUTDENT, Indentation.Punctuation.INDENT, "end",
                               Indentation.Punctuation.OUTDENT, Indentation.Punctuation.OUTDENT),
                     analyze("foo", "\n", 1, "bar", "\n", 3, "baz", "\n", 2, "end"));
    }
    ...
    public void testIndent() {
        ...
        Asserts.assertFailure(mode,
                              parser.from(Parsers.constant(() -> tokenList(Indentation.Punctuation.OUTDENT))),
                              "", 1, 1,
                              "INDENT expected, OUTDENT encountered.");
    }
    ...
    public void testOutdent() {
        ...
        Asserts.assertFailure(mode,
                              parser.from(Parsers.constant(() -> tokenList(Indentation.Punctuation.INDENT))),
                              "", 1, 1,
                              "OUTDENT expected, INDENT encountered.");
    }

    @Test
    public void testLexer() {
        Parser<List<Token>> parser = new Indentation().lexer(Scanners.IDENTIFIER, Indentation.WHITESPACES.optional(null));
        assertEquals(tokenList("foo", 7, "bar", 4, Indentation.Punctuation.INDENT, "baz", 4, Indentation.Punctuation.INDENT,
                               "bah", 1, "bah", Indentation.Punctuation.OUTDENT, Indentation.Punctuation.OUTDENT),
                     parser.parse("foo \\ \n\\\n bar \n  baz\n   bah bah ", mode));
    }
