...
import static com.easyparsingapi.yari.parsec.Asserts.assertParser;
import static com.easyparsingapi.yari.parsec.Parsers.constant;
import static com.easyparsingapi.yari.parsec.Scanners.string;
...
import com.easyparsingapi.yari.parsec.EmptyListParser;
...
import com.easyparsingapi.yari.parsec.WithSource;
...
public class ParserTest extends BaseMockTest {
    ...
    private static final Parser<Integer> INTEGER = Scanners.INTEGER.source().map(Integer::valueOf).label("integer");
    ...
    public void testParse() throws Exception {
        ...
        assertEquals(Integer.valueOf(123), INTEGER.parse(new StringReader("123")));
        try {
            INTEGER.parse(new StringReader("x"));
            fail();
        } 
    ...
    public void testSource() {
        ...
        assertEquals("123", Parsers.tokenType(Integer.class, "int")
                                   .from(INTEGER, Scanners.WHITESPACES)
                                   .source()
                                   .parse("123", mode));
    }
    ...
    @Mock
    Function<Object, Parser<String>> next;
    ...
    public void testNotFollowedBy() {
        ...
        assertEquals((Object) 123, INTEGER.notFollowedBy(COMMA.times(2)).followedBy(COMMA).parse("123,", mode));
    ...
    public void testTimes_range() {
        ...
        assertListParser(isChar('a').asDelimiter().next(isChar('b')).times(1, 2).followedBy(isChar('a')), "aba", 'b');
    ...
    public void testMany1() {
        ...
        assertListParser(isChar('a').asDelimiter().next(isChar('b')).many1().followedBy(isChar('a')), "aba", 'b');
    ...
    public void testAtomic() {
        ...
        assertEquals((Object) 'a', Parsers.or(areChars("ab").atomic(), isChar('a')).parse("a", mode));
    ...
    public void testSepEndBy1() {
        ...
        assertListParser(INTEGER.sepEndBy1(COMMA.next(COMMA).atomic()).followedBy(COMMA), "1,", 1);

        // 0 step partial delimiter consumption
        assertListParser(INTEGER.sepEndBy1(COMMA.asDelimiter().next(COMMA)).followedBy(COMMA), "1,", 1);
        ...
        assertFailure(mode, INTEGER.followedBy(COMMA).sepEndBy1(COMMA), "1,,1", 1, 5, ", expected, EOF encountered.");

        // 0 step partial delimited consumption
        assertListParser(INTEGER.asDelimiter().followedBy(COMMA).sepEndBy1(COMMA).followedBy(string("1")), "1,,1", 1);
    }
    ...
    public void testSepEndBy() {
        ...
        // atomize on delimiter
        assertListParser(INTEGER.sepEndBy(COMMA.next(COMMA).atomic()).followedBy(COMMA), "1,", 1);

        // 0 step partial delimiter consumption
        assertListParser(INTEGER.sepEndBy(COMMA.asDelimiter().next(COMMA)).followedBy(COMMA), "1,", 1);
        ...
        // partial consumption on delimited.
        assertFailure(mode, INTEGER.followedBy(COMMA).sepEndBy(COMMA), "1,,1", 1, 5, ", expected, EOF encountered.");

        // 0 step partial delimited consumption
        assertListParser(INTEGER.asDelimiter().followedBy(COMMA).sepEndBy(COMMA).followedBy(string("1")), "1,,1", 1);
    }
    ...
    @Mock
    Function<Integer, Integer> unaryOp;
    ...
    @Mock
    BiFunction<Integer, Integer, Integer> binaryOp;
    ...
    public void testFrom() {
        ...
        assertFailure(mode, Terminals.CharLiteral.PARSER.from(constant(() -> Arrays.<Token>asList())), "", 1, 1, "character literal expected, EOF encountered.");
        assertListParser(Parsers.ANY_TOKEN_VALUE.many().from(lexeme), "", 'a', 4L);
        assertFailure(mode, Parsers.ANY_TOKEN_VALUE.from(lexeme), "abcde", 1, 3);
    ...
    public void testFrom_throwsOnScanners() {
        assertFailure(mode, string("foo").from(constant(() -> Arrays.asList(new Token(0, 3, "foo")))), "foo", 1, 1, "Cannot scan characters on tokens.");
        assertFailure(mode, isChar('f').from(constant(() -> Arrays.asList(new Token(0, 1, 'f')))), "f", 1, 1, "Cannot scan characters on tokens.");
    }

    @Test
    public void testFrom_withDelimiter() {
        Parser<List<String>> integers = Terminals.IntegerLiteral.PARSER.many();
        Parser<List<String>> parser = integers.from(Terminals.IntegerLiteral.TOKENIZER, Scanners.WHITESPACES);
    ...
    public void testLexer() {
        ...
        assertEquals(Arrays.<Token>asList(new Token(0, 2, 12L), new Token(3, 1, 3L)), parser.parse("12 3  ", mode));
    }
    ...
    private void assertListParser(Parser<? extends List<?>> parser, String source, Object... expected) {
